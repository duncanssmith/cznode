...
# docker run -d --net=isolated_network --name nodeapp -p 3001:3000 -v $(pwd)/logs:/var/www/logs danwahlin/nodeapp
...
# docker run -d -p 3001:3000 --link my-mongodb:mongodb --name nodeapp danwahlin/nodeapp

FROM        node:alpine

LABEL       author="Duncan Smith"
